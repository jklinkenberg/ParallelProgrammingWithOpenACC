...
/* This code runs a stencil on multiple GPUs of a node of a supercomputer, using MPI+OpenMP+OpenMP-offload. For sharing
data across GPUs, it optionally has UVM support. " but this is not needed for the NVC OpenMP implementation.
*/
...
int main(int argc, char *argv[])
{
    ...
    temp1_h = (double *)malloc(sizeof(double) * (ni + 2) * (nj / numProcesses + 2)) temp2_h =
        (double *)malloc(sizeof(double) * (ni + 2) * (nj / numProcesses + 2));
...
#pragma omp master
                    {
                        ...
                        if (!BLOCKING_MPI)
                        {
                            ...
                            if (procID > 0)
                            {
                                ...
                                MPI_Irecv(&temp2[0], LDA, MPI_DOUBLE, procID - 1, 0, MPI_COMM_WORLD,
                                          &requests[numRequests++]);
                                MPI_Isend(&temp2[LDA], LDA, MPI_DOUBLE, procID - 1, 0, MPI_COMM_WORLD,
                                          &requests[numRequests++]);
                            }
                            if (procID < numProcs - 1)
                            {
                                MPI_Irecv(&temp2[(rows + 1) * LDA], LDA, MPI_DOUBLE, procID + 1, 0, MPI_COMM_WORLD,
                                          &requests[numRequests++]);
                                MPI_Isend(&temp2[(rows)*LDA], LDA, MPI_DOUBLE, procID + 1, 0, MPI_COMM_WORLD,
                                          &requests[numRequests++]);
                            }
                        ...
                        else
                        {
                            if (procID < numProcs - 1)
                            {
                                MPI_Send(&temp2[rows], LDA, MPI_DOUBLE, procID + 1, tag1,
                                         MPI_COMM_WORLD); // send from my row rows
                                MPI_Recv(&temp2[rows + 1], LDA, MPI_DOUBLE, procID + 1, tag2, MPI_COMM_WORLD,
                                         &status); // recv into my row rows+1
                            }
                            /*update the upper halo to the host except the first device*/
                            if (procID > 0)
                            {
                                MPI_Recv(&temp2[0], LDA, MPI_DOUBLE, procID - 1, tag1, MPI_COMM_WORLD,
                                         &status); // recv into my row 0
                                MPI_Send(&temp2[1], LDA, MPI_DOUBLE, procID - 1, tag2,
                                         MPI_COMM_WORLD); // send from my row 1
                            }
                        }
                    }
                }
...
#pragma omp target update from(temp1[LDA : rows * LDA])
    ...
    // fprintf(fileOfTimings, "%s\t%d\t%d\t%d\t%d\t%d\t%d\t%f\n", "Heat2D", numProcs, NUM_THREADS, ni, nj, parStrat,
    // opt, end-start); if (fileOfTimings!=NULL) fclose (fileOfTimings);
